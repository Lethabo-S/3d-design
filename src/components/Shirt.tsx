...
export function Shirt({
  ...
  if (pattern && (!patternMap || pattern === 'solid')) {
    console.log('Please select a pattern to apply.');
  }
  let shirtMaterial: JSX.Element;
  if (patternMap && pattern) {
    // If a pattern is selected, use it as the main map
    shirtMaterial = (
      <meshStandardMaterial color={color} map={patternMap} roughness={material === 'silk' ? 0.1 : 0.8} metalness={0} />
    );
  } else if (map) {
    // If a texture is selected (and no pattern), use it
    shirtMaterial = (
